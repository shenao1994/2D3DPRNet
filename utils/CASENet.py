...
def gen_mapping_layer_name(model):
    ...
    """
    layer_to_name_dict = {}  # key is module name in currrent reg_model, value is the filename of numpy
    for (m_name, m) in model.named_parameters():
        if not "res" in m_name:  # This case, name is totally the same.
            ...
            if "downsample.1" in m_name:  # For the BN layer in another branch
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch1_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch1_1"
            ...
            if "bn1" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch2a_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch2a_1"
            if "bn2" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch2b_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch2b_1"
            if "bn3" in m_name:
                if "weight" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch2c_0"
                if "bias" in m_name:
                    layer_to_name_dict[m_name] = m_name.split('.')[0].replace("res",
                                                                              "scale") + label_anno + "_branch2c_1"
    ...
    for k in layer_to_name_dict.keys():
        if ("bn" in k or "downsample.1" in k) and ("weight" in k):
            ...
            avg_name = [layer_to_name_dict[k][:-1].replace("scale", "bn") + "0",
                        layer_to_name_dict[k][:-1].replace("scale", "bn") + "2"]
            var_key_name = k.replace(k.split('.')[-1], "running_var")
            var_name = [layer_to_name_dict[k][:-1].replace("scale", "bn") + "1",
                        layer_to_name_dict[k][:-1].replace("scale", "bn") + "2"]
...
class ScaleLayer(nn.Module):
    ...
    def forward(self, input_data):
        return input_data * self.scale.unsqueeze(0).unsqueeze(2).unsqueeze(3) + self.bias.unsqueeze(0).unsqueeze(
            2).unsqueeze(3)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19):
        ...
        self.res5 = self._make_layer(block, 512, layers[3], stride=1,
                                     special_case=True)  # Notice official resnet is 2, but CASENet here is 1.
        ...
        self.upsample_cls_side5 = nn.ConvTranspose2d(num_classes, num_classes, kernel_size=16, stride=8,
    ...
    def forward(self, x, for_vis=False):
        ...
        cropped_score_feats5 = self.crop_layer(upsampled_score_feats5,
                                               offset=4)  # BS X 19 X 472 X 472. The output of it will be used to get a loss for this branch.
        ...
        fused_feats = self.ce_fusion(
            concat_feats)  # BS X 19 X 472 X 472. The output of this will gen loss for this branch. So, totaly 2 loss. (same loss type)
...
def CASENet_resnet101(num_classes=19):
    """Constructs a modified ResNet-101 reg_model for CASENet.
    Args:
        pretrained (bool): If True, returns a reg_model pre-trained on MSCOCO
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], num_classes)
    # if pretrained:
    #     utils.load_official_pretrained_model(reg_model,
    #                                          "/Users/anirudhchakravarthy/Documents/CodeArchive/Asian-Paints/CASENet-torch-cityscapes/pretrained_models/model_casenet.pth.tar")
    return model
